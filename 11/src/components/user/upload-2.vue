...
export default {
    ...
    methods: {
        ...
        upload() {
            ...
            this.$api.users.QnHeadImg().then( res => {
                var baseData = createImgUrl.substring(createImgUrl.indexOf(",") + 1)
                let client = new XMLHttpRequest();
                client.open('POST', url, true);
                client.responseType = 'json'
                client.setRequestHeader("Content-Type", "application/octet-stream")
                client.setRequestHeader("Authorization", "UpToken " + res.data.token )
                client.onreadystatechange = function() {
                    ...
                    if (this.status === 200 || this.status === 201) {
                        if (that.value) {
                            that.loading = 2;
                            that.$emit('crop-upload-success', this.response, field, ki);
                        }
                    } else {
                        if (that.value) {
                            that.loading = 3;
                            ...
                            that.$emit('crop-upload-fail', "错误", field, ki);
                        }
                    }
                };
