        ...
        handleImageAdded(file, Editor, cursorLocation, resetUploader) {
            if( file ){
                ...
                this.$api.article.gettoken().then( res => {
                    ...
                    render.onload = ( ev ) => {
                        var ImgFile = ev.target.result.substring( ev.target.result.indexOf(",") + 1 )
                        var my_ajax = new XMLHttpRequest()
                        my_ajax.onreadystatechange = () => {
                            if( my_ajax.readyState === 4 ){
                                console.log( my_ajax.response )
                                //http://pj6zsx2jx.bkt.clouddn.com/
                                let url = "https://curtaintan.club/" + my_ajax.response.key
